...
def dm_sum_nn(euclid_distances, k, seed=47):
    ...
    coreset = coreset.union(set(random.sample(list(P_diff_unZ), k - z)))#It avoids warning
...
if __name__ == '__main__':
    ...
    #Diversity Maximization (DM)
    #The goal of this line of research is to provide a summary of a large dataset that preserves the diversity of the data as much as possible
    # Formally, in this task, given a universe P of n items, its goal is to choose a small subset of size k that maximizes a given pre-specified measure of diversity.
    dm_sum_pairwise(euclidean_distances(X), 5)
    dm_min_pairwise(euclidean_distances(X), 5)
    dm_sum_nn(euclidean_distances(X), 5, 12227)

    #s Fair Diversity Maximization (FDM) (Diversity Maximization under Partition Constraints)
    #It is a variant of the problem that has been studied to find a diverse summary while satisfying some additional orthogonal constraints
    #the items in the input are partitioned into m disjoint groups P = P1 ∪ · · · ∪Pm
    #Additionally, one is given a pre-specified number of desired results from each group k1, . . . , km, and the goal is to return ki objects from each group Pi such that the overall diversity among all k = P i ki points is maximized.
    fdm_sum_nn(X, np.argmax(y, axis=1), [5]*n_classes)
    #fdm_min_pairwise(X, np.argmax(y, axis=1), [5]*n_classes, 0.1)
    fdm_sum_pairwise(X, np.argmax(y, axis=1), [5]*n_classes, eps=1e-5)

    #DM vs. FDM
    #When we run FDM, we gain in fairness (or balancedness), but we lose in diversity over the DM expressed in different metrics. 